...
print("const alignas(64) pdata primes[%d] = {"%nprimes)
...
for prime in primes[:nprimes]:
  ...
  u = centermod(pow(2,-28*(nlimbs-1)-6,p),p)
#TODO
  for i in range(1,p-1):
    if 2*i*i % p == 1:
      sqrt2inv = centermod(mont*i,p)
      break
  v = (2**27 + (p-1)//2)//p
  v64 = (2**75 + (p-1)//2)//p
...
print()
print("const alignas(64) qdata modulus = {{")
print("  .q = {{{")
